...
automatically use a particular account depending on the local directory a
project is in.
...
* The git config
  ...
  specify your email address and a url-rewriting [[https://git-scm.com/docs/git-config#Documentation/git-config.txt-urlltbasegtinsteadOf][insteadOf]] line which modifies
...
** More About Git Config Loading
  ...
  *Both* the =work.gitconfig= and =home.gitconfig= are loaded. The email address
  ...
  can set that email address to anything! Set it to
