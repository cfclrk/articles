#+TITLE: Git/SSH Config For Multiple GitHub Accounts
#+SETUPFILE: ../setup.org

* The Problem

  You have multiple GitHub accounts, and you want to git to automatically
  push/pull to the correct GitHub account for each local project.

** Current Solution

   In this article I set things up so that:

   - Projects in your =~/work/= directory use the *work* GitHub account
   - Projects in your =~/projects/= directory use the *home* GitHub account
...
** Unsolved
   ...
   What happens when you clone a repo to =~/some_other_dir=? You have to re-do
   the steps in this article for that directory. What I /really want/ is a
   setup that does this:

   - Projects in =~/work/= and =~/go= use the *work* GitHub account
   - Projects elsewhere use the *home* GitHub account

   Unfortunately, I haven't gotten that working yet (have you? [[https://cfclrk.github.io/][Contact]] me!).
   More about that in the [[#config-loading]] section.
